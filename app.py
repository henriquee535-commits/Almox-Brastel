...
def ajustar_fuso_br(dt_obj):
    if dt_obj: return dt_obj - timedelta(hours=3)
    return None
...
def carregar_ccs():
    ...
    if not lista_cc: lista_cc = ["Geral"]
    return lista_cc
...
# ══════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.usuario_logado:
        ...
        if user['nivel'] in ['Almoxarife', 'Master']: modulos_disp.extend(["📦 Gestão de Estoque", "📱 Telefonia", "📋 Carga por Colaborador"])
        ...
        if modulo_ativo == "🛒 Requisições (RDM/CGM)":
            ...
            with tabs_req[0]:
                ...
                if not lista_colabs:
                    ...
                    with st.form("form_solicitacao", clear_on_submit=True):
                        ...
                        # NOVIDADE: Menu suspenso rastreável
                        retirante = st.selectbox("Colaborador Responsável (Que irá receber/devolver):", lista_colabs)
                        
                        if st.form_submit_button("Enviar Solicitação"):
                            if not cod_req: st.error("Preencha o código.")
                            else:
                                desc_valida = buscar_descricao_por_codigo(cod_req)
                                if not desc_valida: st.error(f"⛔ O código '{cod_req}' não está cadastrado no almoxarifado.")
                                else:
            ...
            if len(abas_req) > 1:
                with tabs_req[1]:
                    ...
                    if not pendentes: st.info("Nenhuma solicitação pendente.")
                    else:
        ...
        # ---------------------------------------------------------
        # MÓDULO NOVO: CARGA POR COLABORADOR
        # ---------------------------------------------------------
        elif modulo_ativo == "📋 Carga por Colaborador":
            st.subheader("🎒 Rastreabilidade de Materiais em Posse")
            st.info("Este módulo calcula automaticamente o saldo de materiais que estão fisicamente com cada colaborador (Soma das Retiradas menos as Devoluções).")
        ...
        # ---------------------------------------------------------
                ...
                with st.form("registro", clear_on_submit=True):
                    ...
                    if st.form_submit_button("✅ Confirmar"):
                        if not cod: st.error("⛔ Informe o Código.")
                            ...
                            if not desc_existente and not desc_input: st.error("⛔ Descrição obrigatória para item novo.")
                            elif desc_existente and desc_input and desc_input.strip() != desc_existente.strip(): st.error(f"⛔ Conflito! Código já é: {desc_existente}")
                            else:
                                with get_conn() as conn:
                                    with conn.cursor() as cur:
            ...
            with tabs_est[1]:
                ...
                if arquivo and st.button("🚀 Processar Importação"):
                    ...
                    if {'Codigo', 'Descricao', 'Quantidade', 'CC'} - set(df_upload.columns): st.error("Colunas inválidas.")
                    else:
                        ...
                        if invalid_ccs: st.error(f"⛔ CCs não encontrados: {', '.join(invalid_ccs)}")
                        else:
                            with get_conn() as conn:
                                with conn.cursor() as cur:
                                    ...
                                    for _, row in df_upload.iterrows():
                                        if (row['Codigo'], row['CC']) in db_set: updates.append((row['Quantidade'], row['Codigo'], row['CC']))
                                        else:
        ...
        # ---------------------------------------------------------
            ...
            with tabs_tel[0]:
                ...
                # (Lógica da telefonia intacta conforme versão anterior para economizar espaço visual)
                st.info("Utilize a carga em massa para atualizações rápidas.")
            with tabs_tel[1]:
                        ...
                        with conn.cursor() as cur:
                            ...
                            for _, r in df_tel_up.iterrows():
                                ...
                                if num_fmt and r['Conta'] in CONTAS_TELEFONIA and (r['CC'] in lista_cc or r['CC'] == ''):
                                    ...
                                    if num_fmt in nums_db: updates_tel.append((r['Conta'], r['Operadora'], r['Colaborador'], r['CC'], st_v, r['Gestor'], num_fmt))
                                    else:
        ...
        # ---------------------------------------------------------
            ...
            with tabs_admin[0]:
                ...
                with st.form("form_usr"):
                    ...
                    n_email, n_senha = uc1.text_input("E-mail:"), uc2.text_input("Senha:", type="password")
                    uc3, uc4 = st.columns(2)
                    n_nivel, n_cc = uc3.selectbox("Nível:", ["Leitor", "Gestor", "Almoxarife", "Master"]), uc4.multiselect("CCs:", ["Todos"] + lista_cc, default=["Todos"])
                    if st.form_submit_button("Criar Usuário") and n_email and n_senha:
                        try:
                            with get_conn() as conn:
                                with conn.cursor() as cur:
                                    cur.execute('INSERT INTO usuarios (email, senha, nome, nivel, cc_permitido) VALUES (%s,%s,%s,%s,%s)', (n_email.lower(), n_senha, n_email.split('@')[0], n_nivel, "Todos" if "Todos" in n_cc else "|".join(n_cc)))
                            conn.commit()
                            st.success("Usuário criado!")
                        except: st.error("Email já cadastrado.")

            with tabs_admin[1]:
                novo_cc = st.text_input("➕ Novo Centro de Custo:")
                if st.button("Adicionar CC") and novo_cc:
                    with get_conn() as conn:
            ...
            with tabs_admin[3]:
                st.warning("⚠️ ÁREA DE RISCO - EXCLUSÕES EM MASSA")
                # Limpezas

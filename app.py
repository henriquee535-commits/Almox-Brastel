...
# ══════════════════════════════════════════════════════════════════════════════
# 3. HELPERS E SISTEMA DE PIN (COM SUPORTE A OUTLOOK/OFFICE365)
# ══════════════════════════════════════════════════════════════════════════════
...
def enviar_email_pin(destinatario, pin):
    try:
        ...
        # Detecção automática do Servidor SMTP (Outlook vs Gmail)
...
def aprovar_acao_master(chave, mensagem):
    ...
    if st.button(f"🔓 Solicitar Desbloqueio", key=f"btn_iniciar_{chave}"):
        ...
        if sucesso:
            ...
            st.error(f"Erro no SMTP do e-mail ({EMAIL_USER}): {erro}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.usuario_logado:
        ...
        # ----------------- REQUISIÇÕES -----------------
            ...
            if len(abas_req) > 1:
                with tabs_req[1]:
                    ...
                    with get_conn() as conn:
                        with conn.cursor() as cur: cur.execute("SELECT id, tipo, data_aprovacao FROM movimentacoes WHERE status = 'Aprovado' ORDER BY data_aprovacao DESC LIMIT 10")
        ...
        # ----------------- TELEFONIA -----------------
            ...
            with tabs_tel[1]:
                ...
                if at and st.button("Importar"):
                    ...
                    with get_conn() as conn:
                        with conn.cursor() as cur:
                            cur.execute('SELECT "Numero" FROM telefonia'); ndb = set(r['Numero'] for r in cur.fetchall())
                            i_t, u_t = [], []
        ...
        # ----------------- ADMINISTRAÇÃO -----------------
            ...
            with tabs_admin[0]:
                with get_conn() as conn:
                    with conn.cursor() as cur: cur.execute("SELECT id, email, nome, nivel, cc_permitido FROM usuarios"); lista_u = cur.fetchall()
